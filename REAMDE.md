...
###CMS widget扩展(实现模块的动态调用)

  模块管理
    - 模块的作用：用于在前台显示扩展内容，例如：导航栏、菜单、最新文章、相关文章、热点文章、最新评论等。
    - Widget扩展: 用于在页面根据需要输出不同的内容，Widget扩展的定义是在ThinkPHP/Lib下。
    - Widget类库需要继承Widget类，并且必须实现render方法。
    

###CMS 模版管理
> 什么是模版
    模版是可复用代码的衍生概念。
    模版分离HTML部分，程序里体现的是完全的业务逻辑。
    模版可以轻松实现更换网站皮肤，进行改版，方便制作不用语言的版本
    模版提高开发速度和性能，是修改和维护更加方便。
    模版支持缓存技术，可以实现网页的静态化
...
ThinkPHP内置了一个基于XML的性能优越模版引擎ThinkTemplate，这是一个专门为ThinkPHP服务的内置模版引擎。ThinkTemplate
是一个使用了XML标签库技术的编译型模板引擎，支持两种类型的模版标签，使用了动态编译和缓存技术，而且支持自定义标签库。
